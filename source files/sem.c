...
int semCreate(key_t key, int sem_num , int sem_flags){          //for semaphore sets sets
  ...
  for( count = 0; count < sem_num; count++ ){
     ...
     if(semctl(sem_id, count, SETVAL, arg) < 0 ) {   //gt oxi icp_stat
          perror("Could not set value of semaphore");
          exit(EXIT_FAILURE);   //alliws return -1
        }
  }
